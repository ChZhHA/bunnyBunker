...
public class DigController : MonoBehaviour
{
        ...
        else
        {
            ...
            if (DiggedHole.Instances.Count > 0)
            {
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    {
                        if (BuildChecker.Instance.CanBuild)
                        {
                            if (mode == OperateMode.Potato)
                            {
                                MakeAPotato(potatoPoint, direction);
                            }
                            else if (mode == OperateMode.Carrot)
                            {
                                MakeACarrot(potatoPoint, direction);
                            }
                        }
                    }
                }
    ...
    public void MakeACarrot(Vector2 position, Vector2 direction)
    {
        var carrot = Instantiate<GameObject>(CarrotPrefebs[Random.Range(0, CarrotPrefebs.Count)], position, Quaternion.identity);
        carrot.transform.SetParent(CarrotContainer.instance.transform);
        carrot.transform.rotation = Quaternion.Euler(0, 0, Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg - 90);
    }
